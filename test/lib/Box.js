  ...
  describe('instance methods', function() {
    describe('_cropMatrix', function() {
      ...
      it('can crop the inside', function() {
        const matrix1 = box._cropMatrix({x: 0, y: 0, width: 2, height: 3});
        ...
        const matrix2 = box._cropMatrix({x: 1, y: 2, width: 2, height: 2});
      ...
      it('should ignore the outside part', function() {
        const matrix1 = box._cropMatrix({x: 2, y: 0, width: 2, height: 2});
        assert.strictEqual(matrix1[0][0].symbol, '3');
        assert.strictEqual(matrix1[1][0].symbol, '6');

        const matrix2 = box._cropMatrix({x: 0, y: 3, width: 2, height: 2});
      ...
      it('should return null if it can not crop at all', function() {
        const matrix = box._cropMatrix({x: 3, y: 0, width: 2, height: 3});
