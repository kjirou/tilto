...
function applyBoxSettingsToMatrix(box/*: Box*/)/*: Matrix*/ {
  ...
  (box.children || [])
    ...
    .forEach(childBox => {
      ...
      contentAreaMatrix = matrixUtils.superposeMatrix(
        contentAreaMatrix,
        preparedChildMatrix,
        {
          x: childBox.x,
  ...
  return matrixUtils.overwriteMatrix(
    box.matrix, contentAreaMatrix, rectangleUtils.toCoordinate(contentArea));
}
...
function toText(
  ...
  return matrixUtils.toText(preparedMatrix, actualOptions.defaultSymbol);
}
